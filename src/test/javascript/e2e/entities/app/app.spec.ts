...
describe('App e2e test', () => {
  ...
  it('should create and save Apps', async () => {
    ...
    const selectedDerivedValue = appUpdatePage.getDerivedValueInput();
    if (await selectedDerivedValue.isSelected()) {
      await appUpdatePage.getDerivedValueInput().click();
      expect(await appUpdatePage.getDerivedValueInput().isSelected(), 'Expected derivedValue not to be selected').to.be.false;
    } else {
      await appUpdatePage.getDerivedValueInput().click();
      expect(await appUpdatePage.getDerivedValueInput().isSelected(), 'Expected derivedValue to be selected').to.be.true;
    }

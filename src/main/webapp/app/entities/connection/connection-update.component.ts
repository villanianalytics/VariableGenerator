...
type SelectableEntity = IApp | ICubes | IVariableName | IListUD;
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ connection }) => {
      ...
      this.listUDService
        .query({ filter: 'connection-is-null' })
        .pipe(
          map((res: HttpResponse<IListUD[]>) => {
            return res.body || [];
          })
        )
        .subscribe((resBody: IListUD[]) => {
          if (!connection.connection || !connection.connection.id) {
            this.connections = resBody;
          } else {
            this.listUDService
              .find(connection.connection.id)
              .pipe(
                map((subRes: HttpResponse<IListUD>) => {
                  return subRes.body ? [subRes.body].concat(resBody) : resBody;
                })
              )
              .subscribe((concatRes: IListUD[]) => (this.connections = concatRes));
          }
        });
    });
  }

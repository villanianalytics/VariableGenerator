...
public class ConnectionResource {
    ...
    public ResponseEntity<List<Connection>> searchConnections(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Connections for query {}", query);
        Page<Connection> page = connectionService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
        }
}
